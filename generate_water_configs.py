...
"""
Generate TOPAS Water phantom simulation configuration files for all 14 energies.
...
# Water template
TEMPLATE = '''# KAVACH BL4S - Water Phantom Validation: {energy} MeV Proton
...
s:Sc/EnergyDep/OutputFile = "Water_EnergyDep_{energy}MeV"
...
s:Sc/Dose/OutputFile = "Water_Dose_{energy}MeV"
...
s:Sc/EnergyDepZ/OutputFile = "Water_EnergyDepZ_{energy}MeV"
...
def main():
    ...
    output_dir = os.path.join(script_dir, "output", "water")
    
    # Create output directory
    os.makedirs(output_dir, exist_ok=True)
    
    print("Generating Water TOPAS configurations...")
    print("=" * 50)
    
    for i, energy in enumerate(ENERGIES):
        seed = 9000 + i * 100 + energy  # Unique seed per energy (9000 series)
        config = TEMPLATE.format(energy=energy, seed=seed)
        
        filename = os.path.join(runs_dir, f"Water_{energy}MeV.txt")
        with open(filename, 'w') as f:
            f.write(config)
        
        print(f"  ✓ Created Water_{energy}MeV.txt (seed={seed})")
    
    print("=" * 50)
    print(f"Generated {len(ENERGIES)} configuration files in runs/")
    print(f"Output directory: output/water/")
    print("\nTo run simulations:")
    print("  bash run_water.sh")
